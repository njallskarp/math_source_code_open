"""Complete interface-7/type62 density114 consumer; 40-vertex necessary kernel."""
import argparse
import errno
import shutil
...
def cases(inputs, certificate):
    ...
    for j, rep in enumerate(family['representatives']):
        for k, mark in enumerate(marks):
            yield (7, 62, j, k), edges, mark, rep['columns']


def specification(edges, mark, columns):
...
def wait_for_storage(directory):
    """Operational guard only; it does not alter a formula or solver limit."""
...
def save_checkpoint(path, records):
    """Keep the previous accepted prefix intact during transient disk exhaustion."""
    ...
    while True:
        ...
        try:
            ...
            if error.errno != errno.ENOSPC:
                raise
            print('Checkpoint storage exhausted; retaining the checked proof and retrying', flush=True)
            time.sleep(5)
...
def main():
    ...
    certificate = audit.load_classification(args.classification)
    rows = list(cases(inputs, certificate))
    expected_keys = {(7, 62, j, k) for j in range(1697) for k in range(2)}
    audit.require(len(rows) == 3394 and {r[0] for r in rows} == expected_keys,
                  'complete cohort keys')
    manifest, runs = [], []
    for key, edges, mark, columns in rows:
        wait_for_storage(scratch)
        stem = '-'.join(map(str, key))
        fixed = specification(edges, mark, columns)
        raw = bytes(48+int(fixed[e]) if e in fixed else 50 for e in PAIRS)
        independently_fixed = audit.matrix(inputs, certificate, key)
        audit.require(raw == independently_fixed, 'physical specifications differ')
        data, variables, clauses = encode(fixed)
        audit.check_support(raw, variables, clauses)
        ...
        save_checkpoint(scratch/'runs.json', runs)
        if not args.keep_native:
            for checked_file in (cnf, other, proof):
                checked_file.unlink()
        if len(runs) % 12 == 0:
            print(len(runs), 'of 3394 cases VERIFIED', flush=True)
    ...
    result = {'status': 'VERIFIED_COMPLETE_INTERFACE7_TYPE62_DENSITY114_EXCLUSION',
              'complete_cases': 3394, 'interface': 7, 'kernel_vertices': 40,
              'physical_free_edges': 389, 'kernel_edge_variables': 272,
