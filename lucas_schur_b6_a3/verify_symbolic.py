#!/usr/bin/env python3
"""Exact affine-cell certificate for the canonical (3,6) KOH remainder.
...
positivity for the canonical (a,b)=(3,6) ray.
"""
...
EXPECTED_CERTIFICATE_SHA256 = "4fab7d452d715e6070e9fdbb07e1d0388dd8037cda6bc0e1ec35e436718741cd"
...
def U(n: int) -> int:
    """Partitions of n using parts 1 and 3."""
    return n // 3 + 1 if n >= 0 else 0


def Q(n: int) -> int:
    """Partitions of n using parts 2 and 3."""
    if n < 0:
        return 0
    if n % 2 == 0:
        return U(n // 2)
...
def gaussian_layer(c: int, i: int) -> int:
    """[q^i](1-q)({c+6 choose 6}_q-{2c+3 choose 3}_q), i<=3c."""
    return (
        P(i)
        - Q(i)
...
def verify_quasipolynomial() -> None:
    ...
    u_series = sum(
        qvar**residue * (
            sp.Rational(1, 3) * (3 * qvar**3 / (1 - qvar**3) ** 2 + residue / (1 - qvar**3))
            + sp.Rational((3, 2, 1)[residue], 3) / (1 - qvar**3)
        )
        for residue in range(3)
    )
    assert sp.cancel(u_series * (1 - qvar) * (1 - qvar**3) - 1) == 0


# A term is (kind, coefficient, (coefficient_of_j, coefficient_of_t, constant)).
...
def p_terms(
    ...
    if constant % 2 == 0:
        half = constant // 2
        return [
            ("T", coefficient, (1, -c_multiplier, half)),
            ("T", coefficient, (1, -c_multiplier, half - 4)),
        ]
    half = (constant - 1) // 2
    return [
        ("T", coefficient, (1, -c_multiplier, half - 1)),
        ("T", coefficient, (1, -c_multiplier, half - 2)),
    ]
...
def q_terms(
    ...
    """Expand Q(2j+r_parity-c_multiplier*c-shift) through U."""
    constant = r_parity - c_multiplier * c_residue - shift
    if constant % 2 == 0:
        return [("U", coefficient, (1, -c_multiplier, constant // 2))]
    half = (constant - 1) // 2
    return [("U", coefficient, (1, -c_multiplier, half - 1))]


def k_terms(r_parity: int, c_residue: int) -> list[Term]:
    """Translate k(c,r), where r=2j for parity 0 or r=2j+1 for parity 1."""
    # The recurrence layer i=r+4 is compared with i-30 at width c-10.
    i_parity = r_parity + 4
    ...
    result += q_terms(i_parity, c_residue, -1, 0, 0)
    result += q_terms(i_parity, c_residue, 1, 0, 30)
    for nu in range(1, 4):
        result += q_terms(i_parity, c_residue, 1, 2, nu)
        result += q_terms(i_parity, c_residue, -1, 2, nu + 10)
    return result
            ...
            for left, right in zip(ordered, ordered[1:]):
                ...
                for kind, coefficient, argument in active:
                    ...
                    if kind == "T" and coefficient > 0:
                        ...
                        bound = argument_expression / sp.Integer(3) + sp.Rational(1, 3)
                    else:
                        bound = argument_expression / sp.Integer(3) + 1
