...
def main():
    parser=argparse.ArgumentParser(description=__doc__);parser.add_argument('--links',type=Path,required=True);parser.add_argument('--forbidden',type=Path,required=True);parser.add_argument('--degree-separator',type=Path,required=True);parser.add_argument('--certificate',type=Path,required=True);parser.add_argument('--deficiency-separator',type=Path,required=True);args=parser.parse_args()
    ...
    with tempfile.TemporaryDirectory() as temp:
        ...
        for damaged in (original.replace('>= -7972','>= -7971'),original.replace('-46 ','-45 ',1),' '.join(original.split()[2:])):
            path.write_text(damaged);reject(lambda:check.deficiency_separator(point,path))
    ...
    check.require(rejected==29,'all corruption controls')
    print(json.dumps({'status':'PASS','physical_transports':transports,'boolean_implication_cases':implication,'corruptions_rejected':rejected,'global_star_boolean_cases':star_cases,'excluded_monochromatic_star_cases':star_violations,'small_complete_emitted_rows':emitted_rows,'conditioned_degree_boolean_cases':degree_cases,'small_complete_degree_rows':degree_rows,'conditioned_triangle_boolean_cases':triangle_cases,'triangle_cap_boolean_cases':cap_cases,'square_expansion_boolean_cases':square_cases,'all_deficiency_partitions':anomaly_cases},sort_keys=True,separators=(',',':')))
