#!/usr/bin/env python3
"""Regenerate the exact complete Q witness and verify its global deficiency separator."""
import argparse
import hashlib
import json
...
def main():
    ...
    controls=run(HERE/'controls.py','--certificate',work/'certificate.json','--links',work/'anchor-links.opbpart','--forbidden',work/'forbidden.opbpart','--degree-separator',work/'degree-separator.opbpart','--deficiency-separator',work/'deficiency-separator.opbpart')
    require(controls==(HERE/'EXPECTED_CONTROLS.json').read_text(),'controls changed');(work/'controls.json').write_text(controls)
    result=run(HERE/'check.py','--certificate',work/'certificate.json','--opb',paths[3323],'--links',work/'anchor-links.opbpart','--forbidden',work/'forbidden.opbpart','--degree-separator',work/'degree-separator.opbpart','--deficiency-separator',work/'deficiency-separator.opbpart')
