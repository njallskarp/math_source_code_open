...
Let (Xin\{A,B\}), and let (S_X,H_X\in\mathbb Z[i]^{21}) be coupled
pointwise through the established (16)-state half-difference/half-sum map.
For every divisor (d\mid21), let (zeta_d) be a primitive (d)-th root of
unity and define the Gaussian cross-trace
...
quarter-turn cell.  If (q) is the total number of quarter-turn cells in
...
Thus all support orbits in a fixed pair ((q,c)) share a single finite
affine cross-trace fiber: it has (6) possible points when (q=5) and
(38) possible points when (q=37).  The real part is (0) in cases
(2,5) and (-4) in cases (0,1,3,4); the imaginary part occupies one
fixed residue class modulo (21).
...
This proves the local claim about (epsilon_j).  It also proves the stated
parity and bound for (sigma), because (sigma) is a sum of exactly (q)
signs in ({-1,1\}).

For arbitrary length-(21) Gaussian words (S,H), expand the cross-trace:
...
where (c_d) is the Ramanujan sum.  Character orthogonality on the cyclic
group (C_{21}) gives
...
The proof is human-checkable; the source provides redundant exact verification.
Interpreter, operating-system, and hardware trust applies only to that
verification.  No floating point, external package, randomness, solver
status, or timeout enters the result.
