...
int main(int argc, char** argv) try {
    if (argc != 10) {
        std::cerr
            << "usage: verify_objective_eleven_first_expansion "
    ...
    const std::array<std::size_t, 11> expected_primary = {
    ...
    if (sources.size() != 372974 ||
        ...
        throw std::runtime_error("invalid objective-eleven frontier array");
    const std::unordered_set<State, StateHash> frontier(
        sources.begin(), sources.end()
    );

    std::array<std::unordered_set<State, StateHash>, 12> expected_additions;
    const std::string expected_text = read_text(argv[8]);
    for (int objective = 0; objective <= 11; ++objective) {
        ...
        for (const State& state : load_states(argv[8], key)) {
            if (!(model.canonical(state) == state) || !model.is_free(state) ||
...
#pragma omp parallel for schedule(dynamic, 8)
    for (std::size_t source_index = 0; source_index < sources.size();
        ...
        if (analysis.objective != 11) ++result.objective_errors;
        ...
        for (int id = 0; id < edge_count; ++id) {
            ...
            if (objective == 11) ++result.new_eleven_incidence;
    ...
    for (const ThreadResult& result : thread_results) {
        ...
        merged.new_eleven_incidence += result.new_eleven_incidence;
    ...
    output << "  \"all_direct_checks_pass\": "
           << ((omitted_targets == 0 && unexpected_targets == 0 &&
                ...
                target_incidence_sum == merged.directed_to_new)
                   ...
                   : "false")
           << ",\n";
    output << "  \"method\": \"independent OpenMP direct enumeration of all 962,598 five-sets for each objective-eleven source, followed by exact cyclic canonicalization and full target-set comparison\"\n}\n";
    return 0;
