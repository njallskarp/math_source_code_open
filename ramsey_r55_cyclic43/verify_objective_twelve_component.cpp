...
int main(int argc, char** argv) try {
    if (argc != 13) {
        std::cerr
            << "usage: verify_objective_twelve_component LOWER-SIX.json "
               ...
               "OBJECTIVE-ELEVEN-COMPONENT.json OBJECTIVE-TWELVE-FRONTIER.json "
    ...
    const std::vector<State> seeds = load_states(
        argv[10], "new_objective_12_rotation_representatives"
    );
    const std::vector<State> expected = load_states(
        argv[11], "complete_additional_objective_12_rotation_representatives"
    );
    if (seeds.size() != 229 || expected.size() != 238)
    ...
    while (next < queue.size()) {
        ...
        if (analysis.objective != 12) ++objective_errors;
    ...
    std::uint64_t outside_above_twelve = 0;
    ...
    for (const State& source : additions) {
        ...
        for (int id = 0; id < edge_count; ++id) {
            ...
            if (objective == 12 && first_frontier_set.contains(key)) {
                ++to_first_frontier;
                continue;
            }
            if (objective == 12 && additions.contains(key)) {
    ...
    output << "  \"all_direct_checks_pass\": "
           << ((omitted_expected_states == 0 && unexpected_states == 0 &&
                ...
                orbit_errors == 0 && additions.size() == 238 &&
                discovered_after_first_expansion == 9)
                   ? "true"
                   : "false")
           << ",\n";
