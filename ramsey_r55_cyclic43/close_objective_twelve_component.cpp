...
void close_component(
    ...
    const std::string& objective_eleven_component_path,
    const std::string& objective_twelve_frontier_path,
    ...
    add_primary(
        11, objective_eleven_frontier_path,
        "objective_eleven_rotation_representatives"
    );
    add_primary(
        11, objective_eleven_component_path,
        "complete_additional_objective_11_rotation_representatives"
    );

    std::vector<State> first_frontier = load_state_array(
        objective_twelve_frontier_path,
        "objective_twelve_rotation_representatives"
    );
    if (first_frontier.size() != 1041887)
        throw std::runtime_error("objective-twelve first frontier mismatch");
    ...
    if (first_frontier_set.size() != first_frontier.size())
        throw std::runtime_error("duplicate objective-twelve frontier state");

    std::array<std::unordered_set<State, StateHash>, 13> additions;
    ...
    if (initial_addition_count != 229 || additions[12].size() != 229)
    ...
    while (next < queue.size()) {
        if (next && next % 10000 == 0)
            std::cerr << "objective-twelve closure: " << next << '/'
        ...
        for (int id = 0; id < edge_count; ++id) {
            ...
            if (objective <= 11 && primary[objective].contains(key)) continue;
            if (objective == 12 && first_frontier_set.contains(key)) continue;
    ...
    std::uint64_t directed_outside_above_twelve = 0;
    int exact_escape_objective = std::numeric_limits<int>::max();
    for (int objective = 0; objective <= 12; ++objective) {
        ...
        for (const State& source : additions[objective]) {
            ...
            for (int id = 0; id < edge_count; ++id) {
                ...
                exact_escape_objective =
                    std::min(exact_escape_objective, target_objective);
                ++directed_outside_above_twelve;
            }
    ...
    const std::uint64_t complete_vertex_count =
        24260213 + orbit_size * (first_frontier.size() + addition_orbits);
    const std::uint64_t complete_edge_count =
        133822192 + orbit_size *
    ...
    output << ",\n  \"complete_objective_twelve_rotation_orbit_count\": "
           << first_frontier.size() + additions[12].size() << ",\n";
    output << "  \"complete_objective_twelve_vertex_count\": "
           << orbit_size * (first_frontier.size() + additions[12].size())
           << ",\n";
    output << "  \"complete_primary_sublevel_twelve_vertex_count\": "
           << complete_vertex_count << ",\n";
    output << "  \"complete_primary_sublevel_twelve_edge_count\": "
           << complete_edge_count << ",\n";
    ...
    output << "  \"undirected_inside_addition_quotient_edges\": "
           << directed_inside_addition / 2 << ",\n";
    output << "  \"directed_outside_above_twelve_from_addition\": "
           << directed_outside_above_twelve << ",\n";
    ...
    output << ",\n  \"exact_escape_objective\": " << exact_escape_objective
           << ",\n";
    bool separator = false;
    for (int objective = 0; objective <= 12; ++objective) {
    ...
    if (separator) output << ",\n";
    output << "  \"method\": \"exact orbit-canonical breadth-first closure from the complete first-expansion addition under every one-edge move of objective at most twelve, followed by a complete reverse edge recount\",\n";
    output << "  \"scope_note\": \"Complete connected threshold-twelve component through the certified Cyclic(43) optimum; disconnected sublevel-twelve components remain out of scope.\"\n}\n";
}
...
int main(int argc, char** argv) try {
    if (argc != 13) {
        std::cerr
            << "usage: close_objective_twelve_component CERTIFICATE.json "
