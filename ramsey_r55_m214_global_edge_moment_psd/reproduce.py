#!/usr/bin/env python3
"""Regenerate the complete global edge PSD survivor and both exact PSD proofs."""
import argparse
import hashlib
import json
...
def main():
    ...
    independent=run(HERE/'independent_global.py',work/'certificate.json')
    ...
    global_controls=run(HERE/'global_controls.py')
    require(global_controls==(HERE/'EXPECTED_GLOBAL_CONTROLS.json').read_text(),'global controls changed');(work/'global-controls.json').write_text(global_controls)
