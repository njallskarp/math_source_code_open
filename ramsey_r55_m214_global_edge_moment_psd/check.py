#!/usr/bin/env python3
"""Exact complete W survivor with global edge-moment positive semidefiniteness."""
import argparse
...
from incident import incident_matrices
from global_psd import global_edge_matrix
...
def neighbor_matrix(point):
    # Every physical entry of M(1,a_0,...,a_42), with exact integers.
    cache={}
    def red_product(e,f):
        vs=tuple(sorted(set(e+f)));pairs=tuple(it.combinations(vs,2))
        mask=sum(1<<pairs.index(z) for z in set((e,f)))
        key=(point.types(vs),mask)
        if key not in cache:
            if len(vs)==2:mass=point.V[point.edge[vs]]
            else:
                dist=point.triples[key[0]] if len(vs)==3 else point.four[key[0]]
                mass=sum(m for s,m in enumerate(dist) if s&mask==mask)
            cache[key]=mass
        return cache[key]
    edges={h:tuple(tuple(sorted((h,u))) for u in sorted(E-{h})) for h in range(N)}
    means=[sum(point.V[point.edge[e]] for e in edges[h]) for h in range(N)]
    # Supplied primal factorization M=v v^T. This restriction certifies this
    # point; it is not claimed to describe every PSD point of the system.
    require(all(x%point.D==0 for x in means),'integral supplied rank-one means')
    factor=[1]+[x//point.D for x in means]
    require(factor==[1]+[7 if h in (29,30) else 6 for h in range(N)],'supplied PSD factor')
    checked=0;trace=0
    for h in range(N):
        for k in range(h,N):
            actual=sum(red_product(e,f) for e in edges[h] for f in edges[k])
            require(actual==factor[h+1]*factor[k+1]*point.D,'physical PSD factor entry '+str((h,k)))
            if h==k:trace+=actual
            checked+=1
    require(checked==946 and trace==1574*point.D,'complete neighbor matrix and trace')
    return {'neighbor_matrix_order':44,'neighbor_matrix_unique_entries':990,
            'neighbor_second_moment_checks':checked,'neighbor_matrix_rank':1,
            'neighbor_psd_factor':factor,'neighbor_covariance_maximum_absolute_entry':'0',
            'neighbor_trace':'1574','previous_cut_mean':'72','previous_cut_square':'0'}


def star_square(point,path):
    h=2;neighbors=tuple(range(3,15))+(29,);offset=6
    names=dict(point.missed);next_name=98759
    for key in sorted(point.all_wedges):
        if key not in names:names[key]=next_name;next_name+=1
    require(next_name==125170,'P3 names')
    coefficients=Counter();constant=offset*offset;mean=0;second=0
    for u in neighbors:
        i=point.edge[tuple(sorted((h,u)))];coefficients[i]+=1-2*offset
        mean+=point.V[i];second+=point.V[i]
    values=dict(enumerate(point.V));values.update({i:point.all_wedges[key] for key,i in names.items()})
    for u,v in it.combinations(neighbors,2):
        a=point.edge[tuple(sorted((h,u)))];b=point.edge[tuple(sorted((h,v)))];w=names[u,v,h]
        coefficients[a]+=2;coefficients[b]+=2;coefficients[w]+=2;constant-=2
        second+=2*(values[w]+point.V[a]+point.V[b]-point.D)
    fields=path.read_text().split();require(fields[-3:]==['>=','120',';'] and len(fields)%2==1,'star-square syntax');actual={}
    for j in range(0,len(fields)-3,2):
        token=fields[j+1];require(token.startswith('x'),'star-square variable');i=int(token[1:]);require(i not in actual,'duplicate star-square term');actual[i]=int(fields[j])
    require(actual==dict(coefficients) and len(actual)==91 and constant==-120,'complete star-square coefficients')
    value=second-2*offset*mean+offset*offset*point.D
    require(sum(c*values[i] for i,c in actual.items())+constant*point.D==value,'literal star-square equality')
    require(value>=0,'former negative star square repaired')
    return {'star_square_center':h,'star_square_neighbors':neighbors,'star_square_offset':offset,
            'star_square_terms':len(actual),'star_square_rhs':120,
            'star_count_mean':str(F(mean,point.D)),'star_count_second_moment':str(F(second,point.D)),
            'star_square_value':str(F(value,point.D)),'former_star_square_status':'NONNEGATIVE',
            'star_square_sha256':hashlib.sha256(path.read_bytes()).hexdigest()}


def mixed_square(point,path):
    # Multiply the affine polynomial directly, independently of the emitter's
    # closed coefficient formula. Lift each distinct edge product to P3/P4.
    weights={tuple(sorted((29,u))):2 for u in range(2,15)};weights[2,8]=1
    names=dict(point.missed);nxt=98759
    for key in sorted(point.all_wedges):
        if key not in names:names[key]=nxt;nxt+=1
    expected=Counter();constant=196;values=dict(enumerate(point.V))
    values.update({i:point.all_wedges[key] for key,i in names.items()})
    for e,w in weights.items():expected[point.edge[e]]+=w*w-28*w
    for (e,c),(f,d) in it.combinations(weights.items(),2):
        coefficient=2*c*d;common=set(e)&set(f)
        if common:
            h=next(iter(common));u,v=sorted((set(e)|set(f))-{h})
            expected[names[u,v,h]]+=coefficient
            expected[point.edge[e]]+=coefficient;expected[point.edge[f]]+=coefficient;constant-=coefficient
        else:
            vs=tuple(sorted(e+f));pairs=tuple(it.combinations(vs,2));mask=sum(1<<pairs.index(z) for z in (e,f))
            start=125170+64*rank_subset(vs,43);dist=point.four[point.types(vs)]
            for state,mass in enumerate(dist):
                if state&mask==mask:expected[start+state]+=coefficient;values[start+state]=mass
    expected={i:c for i,c in expected.items() if c};fields=path.read_text().split();actual={}
    require(fields[-3:]==['>=','436',';'] and len(fields)%2==1,'mixed-square relation')
    for j in range(0,len(fields)-3,2):
        require(fields[j+1].startswith('x'),'mixed-square variable');i=int(fields[j+1][1:]);require(i not in actual,'duplicate mixed-square term');actual[i]=int(fields[j])
    require(actual==expected and len(actual)==270 and constant==-436,'all mixed-square coefficients')
    value=sum(c*values[i] for i,c in actual.items())+constant*point.D
    require(value>=0,'former mixed square repaired')
    return {'mixed_square_center':29,'mixed_square_remote_edge':[2,8],'mixed_square_polynomial':'2*a_29+x_2_8-14',
            'mixed_square_terms':270,'mixed_square_rhs':436,'former_mixed_square_status':'NONNEGATIVE',
            'mixed_square_value':str(F(value,point.D)),'mixed_square_sha256':hashlib.sha256(path.read_bytes()).hexdigest()}


def main():
    parser=argparse.ArgumentParser(description=__doc__);parser.add_argument('--certificate',type=Path,required=True);parser.add_argument('--opb',type=Path,required=True);parser.add_argument('--links',type=Path,required=True);parser.add_argument('--forbidden',type=Path,required=True);parser.add_argument('--degree-separator',type=Path,required=True);parser.add_argument('--deficiency-separator',type=Path,required=True);parser.add_argument('--square',type=Path,required=True);parser.add_argument('--mixed-square',type=Path,required=True);args=parser.parse_args();point=Point(args.certificate)
    result={'status':'EXACT_COMPLETE_GLOBAL_EDGE_MOMENT_PSD_SURVIVOR',
            ...
            **conditioned_degrees(point,args.degree_separator),**triangle_rows(point),**deficiency_bound(point,args.deficiency_separator),**neighbor_matrix(point),**incident_matrices(point),**star_square(point,args.square),**mixed_square(point,args.mixed_square),**global_edge_matrix(point)}
