...
import emit_mixed
import incident
from independent_matrix import bareiss_rank


def main():
    parser=argparse.ArgumentParser(description=__doc__);parser.add_argument('--links',type=Path,required=True);parser.add_argument('--forbidden',type=Path,required=True);parser.add_argument('--degree-separator',type=Path,required=True);parser.add_argument('--certificate',type=Path,required=True);parser.add_argument('--deficiency-separator',type=Path,required=True);parser.add_argument('--square',type=Path,required=True);parser.add_argument('--mixed-square',type=Path,required=True);args=parser.parse_args()
    ...
    # Exhaust every labeled five-vertex graph, every center, every neighbor
    # subset (including empty), and offsets -1..5. Compare the closed emitter
    # with direct integer star counts, including all blue-wedge values.
    square_emitter_cases=0
    for graph in range(1024):
        red={e for j,e in enumerate(pairs) if graph>>j&1}
        for h in range(5):
            others=[v for v in range(5) if v!=h]
            for mask in range(16):
                neighbors=[v for j,v in enumerate(others) if mask>>j&1]
                count=sum(tuple(sorted((h,u))) in red for u in neighbors)
                for offset in range(-1,6):
                    edges,wedges,rhs=emit_square.coefficients(5,h,neighbors,offset)
                    actual=sum(c*int(e in red) for e,c in edges.items())
                    actual+=sum(c*int(tuple(sorted((u,h))) not in red and tuple(sorted((v,h))) not in red) for (u,v,h),c in wedges.items())-rhs
                    check.require(actual==(count-offset)**2 and actual>=0,'literal star-square emitter');square_emitter_cases+=1
    check.require(square_emitter_cases==573440,'complete star-square truth table')
    mixed_cases=0
    for center,neighbors,remote in ((29,tuple(range(2,15)),(2,8)),(4,(0,1),(0,1)),(4,(0,1,2,3),(0,1))):
        edges,wedges,products,rhs=emit_mixed.coefficients(center,neighbors,remote)
        support=[tuple(sorted((center,u))) for u in neighbors]+[remote]
        for bits in range(1<<len(support)):
            red={e for j,e in enumerate(support) if bits>>j&1}
            actual=sum(c*int(e in red) for e,c in edges.items())-rhs
            actual+=sum(c*int(tuple(sorted((u,h))) not in red and tuple(sorted((v,h))) not in red) for (u,v,h),c in wedges.items())
            actual+=4*sum(e in red and f in red for e,f in products)
            count=sum(tuple(sorted((center,u))) in red for u in neighbors)
            check.require(actual==(2*count+int(remote in red)-14)**2,'mixed square Boolean expansion');mixed_cases+=1
    psd_cases=0
    for matrix,rank in (([[0]],0),([[1,2],[2,4]],1),([[0,0,0],[0,2,1],[0,1,2]],2),([[2,1,0],[1,2,1],[0,1,2]],3)):
        check.require(incident.psd_rank(matrix)==rank and bareiss_rank(matrix)==rank,'known exact PSD rank');psd_cases+=1
    rejected=0
    def reject(call):
        nonlocal rejected
        try:call()
    ...
    with tempfile.TemporaryDirectory() as temp:
        ...
        for damaged in (original.replace('>= 120','>= 121'),original.replace('+13 ','+12 ',1),' '.join(original.split()[2:])):
            path.write_text(damaged);reject(lambda:check.star_square(point,path))
    ...
    # Damage one first moment, one diagonal product, and one off-diagonal
    # physical four-state product. The factor checker must reject each.
    idx=point.edge[2,3];point.V[idx]+=1;reject(lambda:check.neighbor_matrix(point));point.V[idx]-=1
    for tables,vertices,state in ((point.triples,(2,3,15),6),(point.four,(2,3,15,31),18)):
        key=point.types(vertices);old=tables[key];changed=list(old);changed[state]+=1;tables[key]=tuple(changed)
        reject(lambda:check.neighbor_matrix(point));tables[key]=old
    for bad in ([[1,2],[2,1]],[[0,1],[1,0]],[[-1]],[[1,0],[1,1]]):
        reject(lambda:incident.psd_rank(bad));reject(lambda:bareiss_rank(bad))
    vertices,matrix=incident.physical_matrix(point,29)
    position={v:i+1 for i,v in enumerate(vertices)}
    groups=[[position[v] for v in cell if v!=29] for cell in point.cells];groups=[g for g in groups if g]
    damaged=[row[:] for row in matrix];damaged[1][2]+=1
    reject(lambda:incident.decompose(damaged,groups))
    reject(lambda:incident.decompose(matrix,groups[:-1]))
    with tempfile.TemporaryDirectory() as tmp:
        path=Path(tmp)/'bad';original=args.mixed_square.read_text()
        for damaged in (original.replace('>= 436','>= 437'),original.replace('+4 ','+5 ',1),' '.join(original.split()[2:])):
            path.write_text(damaged);reject(lambda:check.mixed_square(point,path))
    check.require(rejected==48,'all corruption controls')
    print(json.dumps({'status':'PASS','known_psd_rank_cases':psd_cases,'mixed_square_boolean_cases':mixed_cases,'physical_transports':transports,'boolean_implication_cases':implication,'corruptions_rejected':rejected,'global_star_boolean_cases':star_cases,'excluded_monochromatic_star_cases':star_violations,'small_complete_emitted_rows':emitted_rows,'conditioned_degree_boolean_cases':degree_cases,'small_complete_degree_rows':degree_rows,'conditioned_triangle_boolean_cases':triangle_cases,'triangle_cap_boolean_cases':cap_cases,'square_expansion_boolean_cases':square_cases,'all_deficiency_partitions':anomaly_cases,'star_square_emitter_boolean_cases':square_emitter_cases},sort_keys=True,separators=(',',':')))
