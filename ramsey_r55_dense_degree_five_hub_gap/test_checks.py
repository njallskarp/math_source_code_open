...
def cohort(upstream):
    inputs, certificate = audit.load_inputs(upstream)
    rows = list(consumer.cases(inputs, certificate))
    expected = [(9, 126, j, k) for j in range(29) for k in range(12)]
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='r55-kernel-controls-') as directory:
        ...
        for key in [(1,126,0,0), (9,62,0,0), (9,126,29,0), (9,126,0,12), (9,126,-1,0), (9,126,False,0), (9.0,126,0,0)]:
            try:
        ...
        raw = audit.matrix(inputs, cert, (9,126,0,0))
