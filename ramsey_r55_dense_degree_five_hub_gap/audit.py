...
def matrix(inputs, certificate, key):
    require(type(key) is tuple and len(key) == 4 and all(type(x) is int for x in key) and key[:2] == (9, 126)
            and type(key[2]) is int and 0 <= key[2] < 29
            and type(key[3]) is int and 0 <= key[3] < 12, 'cohort key')
    n, edges = graph6(inputs['interfaces'][9])
    ...
    marks = [q for q in permutations(neighbors)
             if all(((min(q[i], q[j]), max(q[i], q[j])) in edges)
                    == (i < 2 <= j) for i, j in combinations(range(5), 2))]
    require(len(neighbors) == 5 and len(marks) == 12, 'all K2,3 markings')
    families = [f for f in certificate['families'] if f['type'] == 126]
    require(len(families) == 1 and len(families[0]['representatives']) == 29,
            'all equality representatives')
    ...
    require(sum(a[u][v] for u, v in combinations(j_vertices, 2)) == 116,
