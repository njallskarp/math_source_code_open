...
import audit_classification as audit
...
def main():
    ...
    changes=[]
    c=deepcopy(certificate);c['family']['representatives'].pop();changes.append(c)
    c=deepcopy(certificate);c['family']['representatives'][0]['columns'][0]^=1;changes.append(c)
    c=deepcopy(certificate);c['family']['representatives'][0]['orbit_size']-=1;changes.append(c)
    c=deepcopy(certificate);c['cross_edges']=37;changes.append(c)
    c=deepcopy(certificate);c['family']['representatives'][0]['columns'][0]=True;changes.append(c)
    for c in changes:
        try:audit.check_certificate(c,Path('/nonexistent-unused-control-tuples'))
        except ValueError:pass
        else:raise ValueError('damaged classification accepted')
    for bad in ['', 'U', '?@', '\n', '~~~']:
        try:audit.graph6(bad)
        except ValueError:pass
        else:raise ValueError('malformed graph6 accepted')
    total,valid=controls()
    print(json.dumps({'status':'TYPE62_DENSITY114_REDUCTION_CONTROLS_PASS',
        'physical_assignments':total,'valid_local_graphs':valid,
        'certificate_mutations_rejected':len(changes),
        'malformed_graph6_rejected':5},indent=2,sort_keys=True))
