"""Complete interface-6/type62 density114 consumer; 40-vertex necessary kernel."""
import argparse
...
def cases(inputs, certificate):
    ...
    for j, rep in enumerate(family['representatives']):
        for k, mark in enumerate(marks):
            yield (6, 62, j, k), edges, mark, rep['columns']
...
def main():
    ...
    expected_keys = {(6, 62, j, k) for j in range(1697) for k in range(2)}
    ...
    for key, edges, mark, columns in rows:
        ...
        checkpoint = scratch/'runs.json.next'
        checkpoint.write_text(json.dumps(runs, indent=2)+'\n')
        checkpoint.replace(scratch/'runs.json')
    ...
    result = {'status': 'VERIFIED_COMPLETE_INTERFACE6_TYPE62_DENSITY114_EXCLUSION',
              'complete_cases': 3394, 'interface': 6, 'kernel_vertices': 40,
