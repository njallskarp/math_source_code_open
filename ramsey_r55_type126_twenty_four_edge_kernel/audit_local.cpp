...
// then all 2^24 assignments. No CNF parser, producer, or conflict-graph input.
#include <algorithm>
#include <array>
#include <cstdint>
#include <fstream>
...
int main(int argc, char** argv) { try {
    ...
    if (nv != 24) throw std::runtime_error("exactly twenty-four free pairs required");
    ...
    bool unique=true;
    for (std::uint32_t mask=0; mask<(std::uint32_t(1)<<24); ++mask) {
        bool bad=false;
    ...
    std::cout << "{\"complete_assignments\":16777216,\"valid_local_fillings\":" << count
