"""Complete extremal five-column classification, Python bitset producer."""
...
def canonical_family(kind,solutions):
    ...
    return {'type':kind,'labeled_count':len(full),'maximum_edges':115 if kind==62 else 116,
            'full_tuple_sha256':sha256(raw).hexdigest(),'representatives':representatives}

def calculate():
    data=setup(37)
    if len(data[0])!=5593 or max(data[1])!=8:raise ValueError('unexpected complete domain')
    families=[]
    for kind in (62,126):
        answer=enumerate_type(kind,37,data,True)
        if set(answer['cross_histogram'])!={37}:raise ValueError('unexpected density boundary')
        families.append(canonical_family(kind,answer['solutions']))
    return {'minimum_cross_edges':37,'families':families}

if __name__=='__main__':
    result=calculate();expected=json.loads(Path(__file__).with_name('certificate.json').read_text())
    if result!=expected:raise ValueError('complete certificate differs')
    print(json.dumps({'status':'COMPLETE_DENSE_PALEY17_FAMILIES_REGENERATED',
                     'labeled_counts':[f['labeled_count'] for f in result['families']],
                     'isomorphism_classes':[len(f['representatives']) for f in result['families']],
                     'maximum_cross_edges':37},indent=2,sort_keys=True))
