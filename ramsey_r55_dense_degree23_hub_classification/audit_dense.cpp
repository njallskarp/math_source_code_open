// Independent full labeled C4-pair decomposition; no orbit pruning.
#include <algorithm>
#include <array>
#include <bit>
#include <cstdint>
...
void grow(int next,Mask m){
    if(std::popcount(m)>=5)cols.push_back(m);
...
int main(int argc,char**argv){try{
    ...
    const int kind=std::stoi(argv[1]);if(kind!=62&&kind!=126)throw std::runtime_error("invalid type");
    ...
    if(largest!=8||cols.size()!=5593)throw std::runtime_error("unexpected domain");
    ...
    for(std::size_t a=0;a<cols.size();a++){
        int need=std::max(0,13-weight[a]);
        for(int b:entries(intersection(B[a],at_least[need]))){
            ...
            for(int c:entries(pool)){
                int need_d=std::max(0,29-weight[a]-weight[b]-weight[c]);
                if(need_d>8)continue;
                Bits ds=intersection(intersection(pool,B[c]),at_least[need_d]);
                for(int d:entries(ds)){
                    if(d<=c)continue;
                    c4_pairs++;
                    int need_e=std::max(0,37-weight[a]-weight[b]-weight[c]-weight[d]);
