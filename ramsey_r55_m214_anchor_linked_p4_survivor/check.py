#!/usr/bin/env python3
"""Exact full P4+9+83 audit and physical forbidden-state provenance; no producer imports."""
import argparse
...
def base_point(opb,point):
    ...
    return {'base_rows':number,'base_equalities':equalities,'base_bytes':size,'base_sha256':digest.hexdigest(),'physical_root_unit_rows':32287,'physical_five_clause_rows':11970,'total_semantic_source_rows':len(seen),'minimum_base_slack':str(F(minimum,D))}
...
def check_forbidden_file(path,point):
    ...
    require(target is not None and target>F(11,100),'strict physical blue separator')
    require((violations[0],violations[1])==(3813,0),'complete separator census')
    return {'forbidden_state_rows':11970,'forbidden_state_sha256':hashlib.sha256(path.read_bytes()).hexdigest(),
            ...
            'separating_anchor':0,'separating_four':[39,40,41,42],'separating_blue_mass':str(target),
            'separation_gap_strict_lower_bound':'11/100','next_system_rows':15429010,
            'next_system_equalities':4149019,'next_system_status':'UNDECIDED'}


def main():
    parser=argparse.ArgumentParser(description=__doc__);parser.add_argument('--certificate',type=Path,default=HERE/'certificate.json');parser.add_argument('--opb',type=Path,required=True);parser.add_argument('--links',type=Path,required=True);parser.add_argument('--forbidden',type=Path,required=True);args=parser.parse_args();point=Point(args.certificate)
    result={'status':'EXACT_COMPLETE_ANCHOR_LINKED_P4_INTEGRAL_SELECTOR_SURVIVOR',
            'common_denominator':str(point.D),'certificate_sha256':hashlib.sha256(args.certificate.read_bytes()).hexdigest(),
            'total_variables':8023409,'total_rows':15417040,'total_equalities':4149019,
            **check_links(point,args.links),**global_moments(point),**full_four_hull(point),**suffix_rows(point),
            **base_point(args.opb,point),**check_forbidden_file(args.forbidden,point)}
