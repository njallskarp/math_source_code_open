#!/usr/bin/env python3
"""Independent definition-level audit for four simultaneous leaves.

Requires SymPy 1.14.x.  This script imports neither the response formulas nor
the production inertia checker.  It constructs each line graph directly,
computes its adjacency characteristic polynomial over ZZ[x], and counts signs
by Descartes variations, exact here because adjacency roots are all real.
"""
...
def add_four_leaves(graph: c2.Graph, ports: tuple[int, int, int, int]) -> c2.Graph:
...
def main() -> None:
    ...
    for spec in ((4, 5, 1), (4, 5, 3), (5, 5, 1), (5, 5, 3)):
        ...
        assert before == 1
        for ports in combinations_with_replacement(range(len(graph)), 4):
            delta = direct_line_signature(add_four_leaves(graph, ports)) - before
            assert delta <= 0
            distribution[delta] += 1
            checks += 1

    assert sorted(distribution.items()) == [(-4, 2285), (-3, 779), (-2, 488), (-1, 24)]
    record = {
        "coefficient_domain": "ZZ[x]",
        "definition_level_quadruples": checks,
