#!/usr/bin/env python3
"""Independent direct-PAF replay of the q=41 weight-16 terminal frontier."""
...
def exact_h_masks(row: dict[str, str]) -> list[int]:
    ...
    assert b_axis.bit_count() == 16 and signature(b_axis) == e
    b_signs = negative_masks(b_axis, (1, 0))
    assert b_signs.size == 128_700
...
def main() -> None:
    ...
    assert sum(int(row["a12_mask"], 16).bit_count() for row in frontier) == 24
    ...
    assert case_tests == 144 and survivors == 0

    print("implementation=independent_numpy_direct_paf_frontier")
    print("stream_b_rotation_orbits=969")
    print("stream_labeled_b_axes=20349")
    print("exact_h_frontier_b_orbits=12")
    print("exact_h_frontier_axis_orbits=24")
    print("fully_recomputed_frontier_b_orbits=12")
    ...
    print("full_weight16_exclusion=verified")
