...
std::vector<std::uint32_t> weight16_orbit_representatives() {
    std::vector<std::uint32_t> result;
    for (std::uint32_t zeros : fixed_submasks(FULL, 5)) {
        const std::uint32_t value = FULL ^ zeros;
    ...
    assert(result.size() == 969);
    assert(result.size() * N == 20349);
    return result;
}
...
AMaps build_a_maps(unsigned signature, std::uint64_t& direct_checks) {
    ...
    for (unsigned a_half = 0; a_half < (1U << 10); ++a_half) {
        ...
        for (unsigned pair_signs = 0; pair_signs < (1U << 10); ++pair_signs) {
            ...
            if (direct_checks < 4096 && (pair_signs % 257U == 0 || a_half % 127U == 0)) {
...
OrbitResult scan_b_orbit(
    ...
    assert(std::popcount(b_axis) == 16);
    const auto imaginary_choices = fixed_submasks(b_axis, 8);
    const auto real_choices = fixed_submasks(FULL ^ b_axis, 2);
    assert(imaginary_choices.size() == 12870);
    assert(real_choices.size() == 10);
    std::vector<Key70> seventh;
    seventh.reserve(128700);
    for (auto imaginary : imaginary_choices) {
        for (auto real : real_choices) {
            ...
            if (direct_checks < 8192 && ((b_assignments + seventh.size()) % 4093 == 0)) {
...
void deepen_b_orbit(std::uint32_t b_axis, const HighMaps& a_maps, OrbitResult& result) {
    ...
    for (auto& support : supports) support.reserve(128700);
    for (auto imaginary : imaginary_choices) {
        for (auto real : real_choices) {
...
int main(int argc, char** argv) {
    const bool emit_stream = argc == 2 && std::string(argv[1]) == "--stream";
    assert(argc == 1 || emit_stream);
    const auto b_orbits = weight16_orbit_representatives();
    ...
    for (const auto& [signature, masks] : by_signature) {
        ...
        for (auto b : masks) {
            ...
            ++processed;
            if (processed % 50 == 0) std::cerr << "processed_b_orbits=" << processed << "/969\n";
        }
    }
    ...
    std::cout << "b_axis_weight=16\n";
    std::cout << "b_axis_words=20349\n";
    std::cout << "b_rotation_orbits=" << results.size() << '\n';
    std::cout << "b_signatures=" << by_signature.size() << '\n';
    std::cout << "b_exact_sum_assignments_per_orbit=128700\n";
    ...
    std::cout << "seventh_order_weight16_exclusion=" << (orbit_pairs[3] == 0 ? "verified" : "not_obtained") << '\n';
    ...
    std::cout << "exact_weight16_exclusion=" << (orbit_pairs[8] == 0 ? "verified" : "not_obtained") << '\n';
    ...
    std::cout << "full_weight16_exclusion=" << (full_survivors == 0 ? "verified" : "not_obtained") << '\n';
