...
def cohort(upstream, classification):
    inputs, _ = audit.load_inputs(upstream)
    certificate = audit.load_classification(classification)
    rows = list(consumer.cases(inputs, certificate))
    expected = [(8, 62, j, k) for j in range(1697) for k in range(2)]
...
def main():
    ...
    args = p.parse_args()
    result = cohort(args.upstream, args.classification)
    ...
    with tempfile.TemporaryDirectory(prefix='r55-kernel-controls-') as directory:
        ...
        cert = audit.load_classification(args.classification)
        for key in [(6,62,0,0), (8,126,0,0), (8,62,1697,0), (8,62,0,2), (8,62,-1,0), (8,62,False,0), (8.0,62,0,0)]:
            try:
        ...
        changed = scratch/'classification'
        changed.mkdir()
        (changed/'certificate.json').write_bytes((args.classification/'certificate.json').read_bytes()+b' ')
        try:
            audit.load_classification(changed)
        except ValueError:
            rejected += 1
        else:
            raise ValueError('changed density-114 classification accepted')
        raw = audit.matrix(inputs, cert, (8,62,0,0))
