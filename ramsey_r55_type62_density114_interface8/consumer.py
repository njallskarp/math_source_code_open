"""Complete interface-8/type62 density114 consumer; 40-vertex necessary kernel."""
import argparse
import errno
import shutil
...
def cases(inputs, certificate):
    ...
    for j, rep in enumerate(family['representatives']):
        for k, mark in enumerate(marks):
            yield (8, 62, j, k), edges, mark, rep['columns']
...
def main():
    ...
    result = {'status': 'VERIFIED_COMPLETE_INTERFACE8_TYPE62_DENSITY114_EXCLUSION',
              'complete_cases': 3394, 'interface': 8, 'kernel_vertices': 40,
