...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='r55-kernel-controls-') as directory:
        ...
        for field, value in [('forbidden_hyperedges', local['forbidden_hyperedges'][:-1]),
        ...
        for damage in ('count', 'mask', 'red_four'):
            changed = deepcopy(local)
            witness = changed['triple_necessity']['witnesses'][0]
            if damage == 'count':
                changed['triple_necessity']['pair_only_model_count'] -= 1
            elif damage == 'mask':
                witness['mask'] ^= 1
            else:
                witness['red_four'] = [0, 1, 2, 3]
            try:
                local_classification.classify(inputs, consensus, changed,
                                               args.audit_cnf, args.audit_local, scratch)
            except ValueError:
                ...
                raise ValueError('false triple-necessity certificate accepted')
        for raw in [b'2'*230, b'2'*232, b'0'*231, b'2'*231, b'3'*231, b'0'*210+b'2'*21]:
            ...
            rejected += 1
    result.update(status='VERIFIED_SIXTEEN_EDGE_CLASSIFICATION_COVERAGE_AND_KERNEL_CONTROLS',
