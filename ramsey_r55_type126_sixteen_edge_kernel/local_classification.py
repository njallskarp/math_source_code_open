...
def check_triples(local, fixed, variables):
    """Small literal witnesses plus a separate set-based all-subset check."""
    forbidden = [set(e) for e in local['forbidden_hyperedges']]
    pairs = [e for e in forbidden if len(e) == 2]
    triples = [e for e in forbidden if len(e) == 3]
    meeting = [set(e) for e in local['required_meeting_sets']]
    pair_only = []
    for mask in range(65536):
        red = {i+1 for i in range(16) if mask >> i & 1}
        if all(not e <= red for e in pairs) and all(e & red for e in meeting):
            pair_only.append((mask, red))
    recursive = independent_transversals([sorted(e) for e in pairs],
                                        local['required_meeting_sets'])
    audit.require([m for m, _ in pair_only] == recursive, 'pair-only recursion versus subsets')
    record = local['triple_necessity']
    audit.require(len(pair_only) == record['pair_only_model_count'] == 203,
                  'pair constraints alone admit extra fillings')
    witnesses = record['witnesses']
    audit.require(len(witnesses) == 3 and [w['removed_hyperedge'] for w in witnesses]
                  == [sorted(e) for e in triples], 'one witness for every triple')
    for removed, witness in zip(triples, witnesses):
        surviving = [m for m, red in pair_only
                     if removed <= red and all(not e <= red for e in triples if e != removed)]
        mask = min(surviving, key=lambda m: (m.bit_count(), m))
        audit.require(mask == witness['mask'] and mask.bit_count() == witness['minimum_red_positions']
                      and len(surviving) == witness['witnesses_without_this_triple'],
                      'exact single-triple witness')
        audit.require(witness['red_positions'] == [i+1 for i in range(16) if mask >> i & 1],
                      'witness red positions')
        q = witness['red_four']
        audit.require(q == sorted(set(q)) and len(q) == 4 and all(0 <= v < 22 for v in q),
                      'physical red-four vertices')
        audit.require(all((mask >> (variables[e]-1) & 1) if e in variables else fixed[e]
                          for e in combinations(q, 2)), 'literal forbidden red four')
    return record
...
def classify(inputs, consensus, local, executable, local_executable, scratch):
    ...
    audit.require(len(variables) == 16 and list(variables) == list(map(tuple, local['free_common_edges'])),
    ...
    audit.require(forbidden == sorted(forbidden) and len(set(map(tuple, forbidden))) == 23
                  and Counter(map(len, forbidden)) == {2: 20, 3: 3}
                  and all(row == sorted(set(row)) and all(1 <= v <= 16 for v in row)
                          for row in forbidden), 'forbidden hypergraph')
    audit.require(len(meeting) == 3 and meeting == sorted(meeting)
                  and all(len(row) == 2 and row == sorted(set(row))
                          and all(1 <= v <= 16 for v in row) for row in meeting),
    ...
    audit.require(models == local['models'] and len(models) == local['model_count'] == 181,
    ...
    audit.require(native == {'complete_assignments': 65536, 'valid_local_fillings': 181,
    ...
    audit.require(sum(c for e, c in fixed.items() if e[1] < 22) == local['fixed_red_base'] == 101,
                  'density polynomial base')
    necessity = check_triples(local, fixed, variables)
    return {**native, 'literal_local_clauses': len(clauses), 'minimal_clauses': len(minimal),
            'forbidden_pair_count': 20, 'forbidden_triple_count': 3, 'required_meeting_sets': 3,
            'dense_models': dense, 'model_sha256': sha256(stream).hexdigest(),
            'triple_necessity': necessity}
