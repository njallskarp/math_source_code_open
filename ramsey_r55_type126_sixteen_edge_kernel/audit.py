...
def validate_consensus(inputs, consensus):
    require(consensus['interfaces'] == [3, 4], 'covered interfaces')
    ...
    n, first = graph6(inputs['interfaces'][3])
    m, second = graph6(inputs['interfaces'][4])
    ...
    require(difference == [tuple(e) for e in consensus['free_common_edges']]
            and len(difference) == 16, 'exact sixteen disagreements')
...
def build_matrix(edges, certificate, key, other=None):
    ...
    require(raw.count(b'2') == (389 if other is None else 405), 'physical free-edge count')
...
def check_support(raw, variables, clauses, consensus):
    ...
    require(variables == expected and len(variables) == 405, 'all physical variables')
    ...
    require({e for e, i in variables.items() if i in active} == expected_pairs,
            'all sixteen attachment bits and 272 private-core bits')
    require(len(active) == 288 and len(outside) == 117, 'active/outside split')
...
def coverage(inputs, certificate, consensus):
    ...
    for h in (3, 4):
        ...
        p = list(range(43)) if h == 3 else consensus['second_to_first']+list(range(22, 43))
