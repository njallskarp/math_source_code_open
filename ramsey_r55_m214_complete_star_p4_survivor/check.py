#!/usr/bin/env python3
"""Exact full P4+9+83+11970 and all-vertex star audit; no producer imports."""
import argparse
...
def base_point(opb,point):
    ...
    return {'base_rows':number,'base_equalities':equalities,'base_bytes':size,'base_sha256':digest.hexdigest(),'physical_root_unit_rows':32287,'physical_five_clause_rows':11970,'physical_degree_source_rows':43,'total_semantic_source_rows':len(seen),'minimum_base_slack':str(F(minimum,D))}
...
def global_star_rows(point):
    ...
    return {'global_star_rows':count,'global_star_rows_per_anchor':by_anchor,
            ...
            'total_rows_without_redundant_anchor_zero_rows':25043020}


def conditioned_degree_separator(point,path):
    ...
    require((total,violated)==(296184,237486),'complete conditioned-degree census')
    require(witness is not None and witness[1]*2==point.D and witness[2]>3*point.D,'strict global degree separator')
    ...
    return {'conditioned_degree_family_rows':total,'violated_conditioned_degree_rows':violated,
            'degree_separator_triple':list(A),'degree_separator_center':h,'degree_separator_state':state,
            'degree_separator_triple_mass':str(F(witness[1],point.D)),'degree_separator_lhs':str(F(witness[0],point.D)),
            'degree_separator_rhs':'21/2','degree_separator_gap':str(F(witness[2],point.D)),
            'degree_separator_gap_strict_lower_bound':'3','degree_separator_terms':len(actual),
            'degree_separator_sha256':hashlib.sha256(path.read_bytes()).hexdigest(),
            'next_system_rows':25351174,'next_system_equalities':4445203,'next_system_status':'UNDECIDED'}


def main():
    parser=argparse.ArgumentParser(description=__doc__);parser.add_argument('--certificate',type=Path,default=HERE/'certificate.json');parser.add_argument('--opb',type=Path,required=True);parser.add_argument('--links',type=Path,required=True);parser.add_argument('--forbidden',type=Path,required=True);parser.add_argument('--degree-separator',type=Path,required=True);args=parser.parse_args();point=Point(args.certificate)
    result={'status':'EXACT_COMPLETE_ALL_STAR_P4_SURVIVOR',
            'common_denominator':str(point.D),'certificate_sha256':hashlib.sha256(args.certificate.read_bytes()).hexdigest(),
            'total_variables':8023409,'total_rows':25054990,'total_equalities':4149019,
            ...
            **conditioned_degree_separator(point,args.degree_separator)}
