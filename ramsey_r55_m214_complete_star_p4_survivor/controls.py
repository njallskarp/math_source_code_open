...
def main():
    parser=argparse.ArgumentParser(description=__doc__);parser.add_argument('--links',type=Path,required=True);parser.add_argument('--forbidden',type=Path,required=True);parser.add_argument('--degree-separator',type=Path,required=True);args=parser.parse_args()
    ...
    with tempfile.TemporaryDirectory() as temp:
        ...
        reject(lambda:check.conditioned_degree_separator(point,path))
