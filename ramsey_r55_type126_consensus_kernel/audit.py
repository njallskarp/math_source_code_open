...
def validate_consensus(inputs, consensus):
    require(consensus['interfaces'] == [10, 12], 'covered interfaces')
    ...
    m, second = graph6(inputs['interfaces'][12])
    ...
    require(difference == [tuple(e) for e in consensus['free_common_edges']]
            and len(difference) == 12, 'exact twelve disagreements')
...
def build_matrix(edges, certificate, key, other=None):
    ...
    require(raw.count(b'2') == (389 if other is None else 401), 'physical free-edge count')
...
def check_support(raw, variables, clauses, consensus):
    ...
    require(variables == expected and len(variables) == 401, 'all physical variables')
    ...
    require({e for e, i in variables.items() if i in active} == expected_pairs,
            'all twelve attachment bits and 272 private-core bits')
    require(len(active) == 284 and len(outside) == 117, 'active/outside split')
