...
int main(int argc,char**argv){
  ...
  const std::array<int,2> expected_branches{31,52};
  const std::array<int,6> expected_q5{3,6,5,3,8,6},expected_q37{6,10,8,11,11,6};
  assert(keys.size()==83&&branch_rows==expected_branches);assert(covered[0].size()==18&&covered[1].size()==18);
  assert(case_rows[0]==expected_q5);assert(case_rows[1]==expected_q37);
  std::cout<<"rows=83;q5_rows=31;q37_rows=52;q5_orbits=18;q37_orbits=18\nindependent_certificate=verified\n";
}
