...
def main():
    ...
    run([args.cxx, '-std=c++20', '-O3', '-Wall', '-Wextra', '-Wpedantic',
         str(source/'audit_cnf.cpp'), '-o', str(scratch/'audit_cnf')], 'compile.txt')
    run([sys.executable, '-B', str(source/'test_checks.py'), '--upstream', str(upstream),
         '--audit-cnf', str(scratch/'audit_cnf')], 'controls.json', 'EXPECTED_TEST.json')
    ...
    print('VERIFIED_COMPLETE_INTERFACE0_DENSITY116_40_VERTEX_OBSTRUCTION', flush=True)
