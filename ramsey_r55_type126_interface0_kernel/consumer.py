"""Complete interface-0/type126 boundary consumer; 40-vertex necessary kernel."""
import argparse
...
def cases(inputs, certificate):
    ...
    for j, rep in enumerate(family['representatives']):
        for k, mark in enumerate(marks):
            yield (0, 126, j, k), edges, mark, rep['columns']
...
def main():
    ...
    expected_keys = {(0, 126, j, k) for j in range(29) for k in range(12)}
    ...
    result = {'status': 'VERIFIED_COMPLETE_INTERFACE0_TYPE126_DENSITY116_EXCLUSION',
              'complete_cases': 348, 'interface': 0, 'kernel_vertices': 40,
