#!/usr/bin/env python3
"""Regenerate the exact complete Q+G witness and verify its cut-square separator."""
import argparse
import hashlib
import json
...
def main():
    ...
    controls=run(HERE/'controls.py','--certificate',work/'certificate.json','--links',work/'anchor-links.opbpart','--forbidden',work/'forbidden.opbpart','--degree-separator',work/'degree-separator.opbpart','--deficiency-separator',work/'deficiency-separator.opbpart','--square',work/'square.opbpart')
    require(controls==(HERE/'EXPECTED_CONTROLS.json').read_text(),'controls changed');(work/'controls.json').write_text(controls)
    result=run(HERE/'check.py','--certificate',work/'certificate.json','--opb',paths[3323],'--links',work/'anchor-links.opbpart','--forbidden',work/'forbidden.opbpart','--degree-separator',work/'degree-separator.opbpart','--deficiency-separator',work/'deficiency-separator.opbpart','--square',work/'square.opbpart')
    require(result==(HERE/'EXPECTED_RESULT.json').read_text(),'complete result changed: '+result)
    (work/'result.json').write_text(result)
    independent=run(HERE/'independent_square.py',work/'certificate.json')
    ...
    pentagon=run(HERE/'pentagon_projection.py')
    require(pentagon==(HERE/'EXPECTED_PENTAGON.json').read_text(),'pentagon projection changed');(work/'pentagon.json').write_text(pentagon)
