...
Inputs4 reconstruct_inputs4() {
    Inputs4 result;
    for (uint32_t word = 0; word <= WORD_MASK; ++word) {
        if (std::popcount(word) != 17) continue;
    ...
    for (uint16_t bits = 0; bits < 1024; ++bits) {
        ...
        if (std::popcount(b_word) != 4) continue;
        ...
        if (result.labeled[required]) result.entries.push_back({b_word, required, theta});
    }
    assert(result.entries.size() == 10);
    int labeled_pairs = 0;
    int orbit_pairs = 0;
    for (const Entry4& entry : result.entries) {
    ...
    assert(labeled_pairs == 420);
    assert(orbit_pairs == 20);
    return result;
}

Residue h_target4() {
    Residue result;
    for (int shift = 1; shift <= 10; ++shift) set_coordinate(result, shift - 1, {-2, 0});
    return result;
}

long long count_h_sum4(const Entry4& entry, int center) {
...
Residue evaluate_quadratic16(
    uint16_t mask,
    const std::array<Residue, 16>& linear,
    const std::array<std::array<Residue, 16>, 16>& quadratic) {
    Residue result;
    for (int j = 0; j < 16; ++j) if ((mask >> j) & 1) {
        result = residue_add(result, linear[j]);
        for (int k = j + 1; k < 16; ++k) if ((mask >> k) & 1) {
...
AData4 enumerate_a4(uint32_t support) {
    ...
    assert(result.exact_assignments == 36);
    return result;
}
...
struct Pair4 {
    ...
    uint32_t b_word = 0;
    auto operator<=>(const Pair4&) const = default;
};

struct BData4 {
    std::unordered_set<Residue, ResidueHash> complements6;
    std::unordered_set<Residue, ResidueHash> complements7;
    long long exact_assignments = 0;
    long long direct_evaluations = 0;
    long long quadratic_audits = 0;
};

BData4 enumerate_b4(const Entry4& entry) {
    ...
    assert(position_count == 16);

    BData4 result;
    result.complements6.reserve(700000);
    result.complements7.reserve(700000);
    Residue target = h_target4();
    constexpr std::array<uint16_t, 16> AUDITS = {
        0x0000u, 0xffffu, 0x5555u, 0xaaaau,
        0x00ffu, 0xff00u, 0x0f0fu, 0xf0f0u,
        0x1234u, 0x2345u, 0x3456u, 0x4567u,
        0x0249u, 0x2492u, 0x9249u, 0x3333u,
    };

    for (uint16_t axes = 0; axes < 256; ++axes) {
        Word baseline_word{};
        baseline_word[0] = {1, 0};
        int pair_index = 0;
        for (int shift = 1; shift <= 10; ++shift) {
            ...
            baseline_word[N - shift] = active(axis ^ entry.theta[shift - 1], 0);
            ++pair_index;
        }
        assert(pair_index == 8);

        Residue baseline = paf_residue(baseline_word);
        std::array<Residue, 16> linear{};
        std::array<std::array<Residue, 16>, 16> quadratic{};
        for (int j = 0; j < 16; ++j) {
        ...
        for (int j = 0; j < 16; ++j) for (int k = j + 1; k < 16; ++k) {
            Word changed = baseline_word;
            changed[positions[j]] = scale(changed[positions[j]], -1);
        ...
        for (uint16_t mask : AUDITS) {
            ...
            assert(direct == evaluate_quadratic16(mask, linear, quadratic));
            ++result.direct_evaluations;
            ++result.quadratic_audits;
        }

        G baseline_sum{1, 0};
        std::array<G, 16> changes{};
        for (int j = 0; j < 16; ++j) {
        ...
        for (uint16_t mask = 1; mask < 256; ++mask) {
            ...
            for (int k = j + 1; k < 16; ++k) if ((rest >> (k - 8)) & 1) {
                value = residue_add(value, quadratic[j][k]);
            }
        ...
        std::array<std::array<Residue, 256>, 8> cross_columns{};
        for (int j = 0; j < 8; ++j) {
            for (uint16_t mask = 1; mask < 256; ++mask) {
                int local = std::countr_zero(mask);
                uint16_t rest = mask & (mask - 1);
        ...
        for (uint16_t left = 0; left < 256; ++left) {
            ...
            for (uint8_t right : found->second) {
                ...
                result.complements6.insert(reduce_sixth(complement));
                result.complements7.insert(reduce_seventh(complement));
                ++result.exact_assignments;
            }
        }
    }
    assert(result.exact_assignments == 1317824);
    assert(result.direct_evaluations == 256LL * (16 + 120 + 16));
    assert(result.quadratic_audits == 256LL * 16);
    return result;
}
...
int main() {
    Inputs4 inputs = reconstruct_inputs4();
    std::map<uint32_t, AData4> a_data;
    std::map<uint32_t, std::vector<int>> by_support;
    for (int entry_index = 0; entry_index < int(inputs.entries.size()); ++entry_index) {
        const Entry4& entry = inputs.entries[entry_index];
        for (uint32_t support : inputs.representatives[entry.required_signature]) {
            by_support[support].push_back(entry_index);
            if (!a_data.contains(support)) a_data.emplace(support, enumerate_a4(support));
        }
    }
    assert(by_support.size() == 16);
    size_t a6_min = SIZE_MAX;
    size_t a6_max = 0;
    size_t a7_min = SIZE_MAX;
    size_t a7_max = 0;
    for (const auto& [support, data] : a_data) {
        (void)support;
        a6_min = std::min(a6_min, data.values6.size());
        a6_max = std::max(a6_max, data.values6.size());
        a7_min = std::min(a7_min, data.values7.size());
        a7_max = std::max(a7_max, data.values7.size());
    }
    int input_pairs = 0;
    ...
    assert(input_pairs == 20);

    std::vector<Pair4> frontier6;
    std::vector<Pair4> frontier7;
    ...
    long long direct_evaluations = 0;
    long long quadratic_audits = 0;
    for (const Entry4& entry : inputs.entries) {
        assert(count_h_sum4(entry, 1) == 1317824);
        assert(count_h_sum4(entry, -1) == 0);
        BData4 b_data = enumerate_b4(entry);
        b6_min = std::min(b6_min, b_data.complements6.size());
        b6_max = std::max(b6_max, b_data.complements6.size());
        b7_min = std::min(b7_min, b_data.complements7.size());
        b7_max = std::max(b7_max, b_data.complements7.size());
        direct_evaluations += b_data.direct_evaluations;
        quadratic_audits += b_data.quadratic_audits;
        for (uint32_t support : inputs.representatives[entry.required_signature]) {
            bool feasible6 = false;
            for (const Residue& value : a_data.at(support).values6) {
                if (b_data.complements6.contains(value)) {
                    feasible6 = true;
                    break;
                }
            }
            ...
            for (const Residue& value : a_data.at(support).values7) {
                if (b_data.complements7.contains(value)) {
                    feasible7 = true;
                    break;
                }
            }
    ...
    assert(frontier6 == std::vector<Pair4>({{503807, 21120}, {524207, 21120}}));
    assert(frontier7.empty());

    std::cout << "reflected_b_masks=10\n"
              << "labeled_type_pairs=420\n"
              << "input_rotation_orbit_pairs=" << input_pairs << '\n'
              << "distinct_a_supports=" << by_support.size() << '\n'
              << "h_b_positive_center_assignments=1317824\n"
              << "h_b_negative_center_assignments=0\n"
              << "h_a_zero_sum_assignments_per_support=36\n"
    ...
    for (const Pair4& pair : frontier6) {
