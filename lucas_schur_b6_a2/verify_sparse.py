#!/usr/bin/env python3
"""Definition-level sparse Z[e1,e2] audit for canonical Lucas (2,6)."""
...
def gaussian_layers(c: int) -> list[int]:
    difference = q_add(gaussian(c + 6, 6), q_neg(gaussian(3 * c + 2, 2)))
    return [
...
def main() -> None:
    ...
    for c in range(6, args.max_c + 1):
        degree = 6 * c
        direct = add(choose[3 * c + 2][2], neg(choose[c + 6][6]))
        ...
        assert min(coefficients[3:]) > 0, ("Schur sign", c)

        if c < 16:
            continue
        previous_layers = gaussian_layers(c - 10)
        k_layers = [
            layers[r + 3] - q_value(previous_layers, r - 27)
            for r in range(3 * c - 2)
        ]
        remainder = lucas_image(k_layers, degree - 6, lucas, 1)
        recurrence = add(e2_shift(differences[c - 10], 30), e2_shift(remainder, 3))
        assert direct == recurrence, ("literal KOH recurrence", c)
        remainder_coefficients = schur_coefficients(remainder, degree - 6)
