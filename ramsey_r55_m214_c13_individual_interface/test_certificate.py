#!/usr/bin/env python3
"""Deterministic rejection tests for the individual-interface checker."""

from __future__ import annotations

import bisect
import copy
import json
import subprocess
import sys
import tempfile
from pathlib import Path


ROOT = Path(__file__).resolve().parent
BASE = json.loads((ROOT / "certificate.json").read_text(encoding="ascii"))


def rejected(name: str, transform) -> None:
    candidate = copy.deepcopy(BASE)
    transform(candidate)
    with tempfile.TemporaryDirectory(prefix="r55-individual-interface-") as temporary:
        path = Path(temporary) / "bad.json"
        path.write_text(json.dumps(candidate), encoding="ascii")
        result = subprocess.run(
            [sys.executable, str(ROOT / "check_certificate.py"), str(path)],
            check=False,
            stdout=subprocess.DEVNULL,
            stderr=subprocess.DEVNULL,
        )
    if result.returncode == 0:
        raise AssertionError(name)
    print(f"REJECTED {name}")


def insert_edge(data, edge: list[int]) -> None:
    bisect.insort(data["outside_red_edges"], edge)


def remove_edge(data, edge: list[int]) -> None:
    data["outside_red_edges"].remove(edge)


def main() -> None:
    rejected("nontransversal", lambda data: data["footprints"].__setitem__(0, "0000"))
    rejected("wrong-mark", lambda data: data["outside_e"].__setitem__(0, 6))
    rejected("wrong-pivot", lambda data: data.__setitem__("pivot", "C0"))
    rejected("forbidden-red", lambda data: insert_edge(data, [0, 6]))
    rejected("forbidden-blue", lambda data: remove_edge(data, [0, 9]))
    print("ALL 5 NEGATIVE CONTROLS REJECTED")


if __name__ == "__main__":
    main()
