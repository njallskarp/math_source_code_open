...
def maximum_block_relaxation(n: int, allow_k26: bool) -> tuple[int, tuple[int, ...]]:
...
def main() -> None:
    ...
    forced_k26: dict[int, int] = {768: 0, 769: 0}
    ...
    for row, profiles in PROFILES.items():
        ...
        for profile in profiles:
            ...
            if n_low == 52:
                ...
                forced_k26[row] += 1
                status = (
                    f"SURVIVES_FORCE_UNIQUE_K26 floor_eL={floor}>"
                    f"no_K26_ceiling={no_k26_ceiling} core_in_block=22..25"
                )
            else:
                surviving[row].append(profile)
    ...
    assert len(surviving[768]) == 1 and forced_k26[768] == 1
    assert len(surviving[769]) == 6 and forced_k26[769] == 3
    output.append("summary row=768 survivors=1 forced_unique_K26=1 eliminated=2")
    output.append("summary row=769 survivors=6 forced_unique_K26=3 eliminated=2")
    digest = sha256(("\n".join(output) + "\n").encode()).hexdigest()
